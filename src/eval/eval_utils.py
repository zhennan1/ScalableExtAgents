...
DATA_NAME_TO_PATH_FIRST = {
    "longbook_qa_eng": "longbook_qa_eng_first.jsonl",
}

DATA_NAME_TO_PATH_LAST = {
    "longbook_qa_eng": "longbook_qa_eng_last.jsonl",
}

DATA_NAME_TO_PATH_RANDOM = {
    "longbook_qa_eng": "longbook_qa_eng_random.jsonl",
}

DATA_NAME_TO_PATH_ALL = {
    "longbook_qa_eng": "longbook_qa_eng_all.jsonl",
}
...
def load_data(data_name: str, mode="default", data_dir: str = "../data/"):
    path = DATA_NAME_TO_PATH[data_name]
    if mode == "first":
        path = DATA_NAME_TO_PATH_FIRST[data_name]
    elif mode == "last":
        path = DATA_NAME_TO_PATH_LAST[data_name]
    elif mode == "random":
        path = DATA_NAME_TO_PATH_RANDOM[data_name]
    elif mode == "all":
        path = DATA_NAME_TO_PATH_ALL[data_name]
...
def create_prompts(tokenizer, eg: dict, data_name: str, model_name: Optional[str], data_dir, chunk_size) -> list[str]:
    prompts = []
    context = eg["context"]
    ...
    if tokenizer:
        ...
        chunked_tokens = chunk_input(tokens, chunk_size)
        chunks = [tokenizer.decode(chunked_token) for chunked_token in chunked_tokens]
    else:
        chunks = chunk_input(context, chunk_size)
...
def create_prompts_new(tokenizer, eg: dict, data_name: str, model_name: Optional[str], data_dir, chunk_size, input_length) -> list[str]:
    prompts = []
    context = eg["context"]
    ...
    if tokenizer:
        ...
        tokens = truncate_input(tokens, input_length, manner="middle")
...
def create_msgs(
    ...
    if tokenizer:
        tokens = tokenizer.encode(prompt)
        print(f"Before truncation: {len(tokens)}")
        tokens = truncate_input(tokens, 8_000 - 1000, manner="middle")
        print(f"After truncation: {len(tokens)}")  # type: ignore
        prompt = tokenizer.decode(tokens)
...
def create_chunked_msgs(
    tokenizer, eg: dict, data_name: str, data_dir, chunk_size, model_name: Optional[str] = None
) -> list[tuple[list[dict], str]]:
    """
    Create chunked messages for a given example.
    """
    chunked_prompts = create_prompts(tokenizer, eg, data_name, model_name, data_dir, chunk_size)

    # Check if tokenizer is provided and initialized
    # if tokenizer:
    #     tokens = tokenizer.encode(prompt)
    #     print(f"Total tokens: {len(tokens)}")
    #     chunks = chunk_input(tokens, chunk_size=7900)
    #     chunked_prompts = [tokenizer.decode(chunk) for chunk in chunks]
    # else:
    #     chunked_prompts = chunk_input(prompt, chunk_size=7900)

    chunked_msgs = []
    for chunked_prompt in chunked_prompts:
        if data_name == "math_calc":
            msgs = [
                {"role": "system", "content": create_system_msg(data_name)},
                {"role": "user", "content": "1 + 2 - 4 - 10"},
        ...
        chunked_msgs.append((msgs, chunked_prompt))
    # print(chunked_msgs)
    return chunked_msgs


def create_chunked_msgs_new(
    tokenizer, eg: dict, data_name: str, data_dir, chunk_size, input_length, model_name: Optional[str] = None
) -> list[tuple[list[dict], str]]:
    """
    Create chunked messages for a given example.
    """
    chunked_prompts = create_prompts_new(tokenizer, eg, data_name, model_name, data_dir, chunk_size, input_length)
...
def chunk_input(input, chunk_size):
    """
    Splits the input text into chunks of specified size.

    Args:
        input (str): The input text to be chunked.
        chunk_size (int): The size of each chunk. Default is 8000.
